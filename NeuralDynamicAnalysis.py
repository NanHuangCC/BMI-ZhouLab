...
Us0t, Um0t, X0t, Y0t = dataProcess_BMI.ReadBehNeu(Beh_path= Beh_path, Neu_path= Beh_path, TauT= 3, FramePerS= 5)

# generate shuffle IDX
RamIdx1 = torch.randperm(X0t.shape[0])
RamIdx2 = torch.randperm(X0t.shape[0])
RamIdx3 = torch.randperm(X0t.shape[0])
# Chose Neuron to predict
for NeuID in range(X0t.shape[1]):
    ...
    Xt = X0t[:, NeuID:(NeuID + 1)]
    Yt = Y0t[:, NeuID:(NeuID + 1)]
    # select shuffle part ###########################################################################################
    for ShuffleType in range(6):
        Xt = X0t[:, NeuID:(NeuID + 1)]
        Ust = Us0t[:, :, :, :]
        Umt = Um0t[:, :, :, :]
        if ShuffleType == 1:
        ...
        for timerange in range(4):
            ...
            # Network define ##########################################################################################
            model = NetModel.NeuActRegNET(in_channels2DS=1, out_channel2DS=1, kernel_size2DS=kernel_size,
                                          stride2DS=stride,
                                          in_channels1DS=in_channels1D, out_channels1DS=out_channels1D,
                                          kernel_size1DS=kernel_size1D, stride1DS=stride1D,
                                          in_channels2DM=1, out_channel2DM=1, kernel_size2DM=kernel_size,
                                          stride2DM=stride,
            ...
            for epoch in range(num_epoch):
                ...
                if epoch % 200 == 0:
                    ...
                    print("epoch{},train_loss:{},test_loss:{}, MSE_train{},MSE_test{}".format(epoch, loss_train,
                                                                                              loss_test,
                                                                                              MSE_train, MSE_test))
            ...
            train_log.to_csv(f'{NetPath}/log_Neu{NeuID}_Shuf{ShuffleType}_T{timerange}.csv')
            torch.save(model, f"{NetPath}/model_Neu{NeuID}_Shuf{ShuffleType}_T{timerange}.pkl")
            ...
            if ShuffleType == 0:
                ...
                for i in range(shap_values.shape[2]):
                    np.savetxt(f'{NeuPath}/shape_Neu{NeuID}_Shuf{ShuffleType}_T{timerange}.csv', shap_values[:, :, i],
                               delimiter=",")
            ...
            Full_result.to_csv(f'{NeuPath}/FeaturesOut_Neu{NeuID}_Shuf{ShuffleType}_T{timerange}.csv')
